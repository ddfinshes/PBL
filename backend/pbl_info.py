...
# 默认 3（偏简单），可被前端通过 update_pbl_info 动态覆盖
...
def update_pbl_info(
    ...
    # 将故事难度写入模块级变量，供认知负荷计算使用
    pbl_story_difficult = story_difficult

    print(
        f"PBL Info Updated: Scene {scene_idx}, Question {q_idx}, story_difficult {story_difficult}, {len(questions)} questions loaded."
    )
...
def update_pbl_info(story: str, questions: list, story_difficult: int = 3, scene_idx: int = 0, q_idx: int = 0):
    """更新全局 PBL 案例信息，供 Agent 讨论使用"""
