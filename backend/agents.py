...
def format_persona_to_string(persona: Dict) -> str:
    ...
    aa = persona.get('cognitive_orientation').get('attentional_anchor')
    merge_attentional_anchor = [attentional_anchor[a] for a in aa]

    reasoning_entry = {
        "mechanism": '模式经验驱动推理方式： 推理起点：从熟悉或常见病例出发；典型思路：通过相似案例快速联想，快速匹配模式；潜在局限：容易过早下结论，可能忽略不典型表现',
        "external_factors": '机制联想起点推理方式： 推理起点：基于器官或病理机制；典型思路：强调生理和病理解释，推理过程复杂但逻辑严密；潜在局限：推理链条较长，不易快速收敛到诊断',
        "risk_perception": '风险优先起点推理方式：推理起点：从最危险的可能性开始；典型思路：优先排除严重后果，确保安全；潜在局限：讨论范围受限，可能忽略非紧急病因',
        "familiarity_driven": '整体状态起点：推理起点：从个体整体状态（如体质或长期状态）出发；典型思路：从全身或长期健康状态解释症状；潜在局限：诊断指向不明确，可能缺乏特异性'
    }
    re = persona.get('cognitive_orientation').get('reasoning_entry')
    merge_reasoning_entry = [reasoning_entry[r] for r in re]

    causal_strucure = {
        "linear_causality": '单线因果模型：推理方式：用单一原因解释全部症状；典型表现：结论明确、推理快速，适合典型病例；常见问题：容易忽略冲突证据，对复杂情况解释力不足',
        "multi_concurrent": '并列多因模型：推理方式：多因素并列罗列，不强调主次；典型表现：全面列出多种可能性，避免遗漏；常见问题：缺乏整合与收敛，难以形成明确诊断方向',
        "cues_driven": '启发式线索整合：推理方式：基于关键线索快速联想，抓住典型特征；典型表现：快速匹配模式，适合经验丰富的医生；常见问题：机制解释不完整，可能忽略非典型表现',
        "undefined": '心理–社会因果模型：推理方式：侧重非生物医学解释，强调心理或环境因素；典型表现：从患者心理状态或社会环境寻找病因；常见问题：可能偏离医学主线，忽略器质性病变'
    }
    cs = persona.get('cognitive_orientation').get('causal_structure')
    merge_causal_strucure = [causal_strucure[c] for c in cs]
    ...
    - 领域知识深度：
        - 教科书级理解：{persona.get('knowledge_background').get('high')} \n
            表现：能给出 {persona.get('knowledge_background').get('high')} 的标准解释，但可能不敏感于罕见/关键细节。\n
        - 知道术语但理解松散：{persona.get('knowledge_background').get('mmedium')}。 \n
            表现：能提 {persona.get('knowledge_background').get('mmedium')} 名词，但机制模糊或泛化。\n
        - 仅生活常识：{persona.get('knowledge_background').get('low')}。\n
            表现：只用日常因果解释这些知识或者现象 {persona.get('knowledge_background').get('low')}（如“吃多了对身体不好”）。\n

    - 认知维度（作用：决定 agent“从哪里开始想、怎么想，发言保留可能存在的缺陷”）：\n
        - 注意力锚点：该学生agent习惯重点关注患者/案例的以下方面：{merge_attentional_anchor}。 \n
        - 推理起点类型：该学生agent习惯从以下几个角度进行思考：{merge_reasoning_entry}。\n
        - 逻辑推理方式：该学生agent通常采用以下几种思考方式：{merge_causal_strucure}。\n

    - 社会行为维度（作用：决定 agent“怎么说、怎么影响他人”）\n
        - 发言风格: {verbal_confidence[persona.get('social_interaction_style').get('verbal_confidence')]} \n
        - 发言专业用语情况：{language_register[persona.get('social_interaction_style').get('language_register')]} \n
        - 与其他同学互动特点：{interaction_role[persona.get('social_interaction_style').get('interaction_role')]} \n

    - 动态学习维度（作用：决定在讨论中吸收知识的速度，“能否被教会”）\n
        - 随着讨论的深度思维的转变情况：{learning_adaptivity[persona.get('learning_adaptivity')]}
    """)
